...
// Called when status of player changes
function onPlayerStateChange(){
    ...
    //When user press pause it stopes to check current time
...
// when the time changes, this will be called.
    function onProgress(currentTime) {
        ...
        /**
         *  Compares with currentTime all elements from array with emotions and
         *  shows appropriate element or elements
         */
        for(i = 0; i < ci; i++){
            if(currentTime === array_of_json[i].time && currentTime != 0) {
                $(document).ready(function(){

                    $( '#emoticons' ).prepend( "<div class='emotion " + array_of_json[i].emotion
                    +" "+array_of_json[i].time+"' style='display: none'></div>" );
                    (function(){
                        var selector = 'div.'+array_of_json[i].time;
                        $(selector).show(1000);
                        $(selector).fadeOut(10000, function(){$(selector).remove();});
                    })();
    ...
    $('.emoticon_button').click(function(){
        ...
        $.ajax({
            ...
            $(document).ready(
                ...
                $('div#emo_animate div').animate({left: '50%', opacity: '0,5'},2000);
                (function(){
